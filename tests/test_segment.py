...
# CSF is an optional extra; its segmenter is only registered when the package is there.
needs_csf = pytest.mark.skipif(
    importlib.util.find_spec("CSF") is None,
    reason="cloth-simulation-filter not installed",
)
...
@pytest.mark.parametrize("name", ["ground_grid", pytest.param("csf", marks=needs_csf)])
...
def test_segmenter_specs_flag_gravity_for_ground_filters():
    ...
    assert gravity["ransac_ground"] and gravity["ground_grid"]
    if "csf" in gravity:  # only registered when cloth-simulation-filter is installed
        assert gravity["csf"]
