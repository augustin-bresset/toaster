...
class PointCloud:
    ...
    def from_source_frame(self, values: np.ndarray) -> np.ndarray:
        """Subset an original-frame-length ``values`` down to the cloud's kept points.

        The inverse of :meth:`to_source_frame`: when points were dropped at load,
        an array that spans the whole on-disk frame (e.g. a label channel written
        against it) is gathered onto the rows the cloud actually kept, so it lines
        up index-for-index with ``xyz``. A no-op when nothing was dropped.
        """
        values = np.asarray(values)
